  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing:border-box; } html { scroll-behavior:smooth; } body { margin:0; color:var(--ink); background:var(--canvas); } a { color:inherit; }
      ...
      @media (max-width:720px) { .wrap { width:min(100% - 2rem,1180px); } .hero { padding-top:4.6rem; } h1 { font-size:clamp(2.35rem,11.5vw,3.45rem); } .preview-head { align-items:start; flex-direction:column; } iframe { height:560px; min-height:0; } .why { grid-template-columns:1fr; } .why h2 { max-width:11ch; } }
    </style>
  </head>
  <body>
    ${renderHeader(null)}
